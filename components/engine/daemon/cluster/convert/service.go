...
func serviceSpecFromGRPC(spec *swarmapi.ServiceSpec) (*types.ServiceSpec, error) {
	...
	convertedSpec := &types.ServiceSpec{
		...
		SoftBorne:    SoftBorneFromGRPC(spec.SoftBorne),
	}
...
// SoftBorneFromGRPC converts a SoftBorne to a GRPC SoftBorne
func SoftBorneFromGRPC(softborne swarmapi.SoftBorne) *types.SoftBorne {
	if len(softborne) <= 0 {
		return &types.SoftBorne{}
	}
	sb := make(types.SoftBorne)
	sb["hard"] = types.Borne{
		Actual: softborne["hard"].Actual,
		Value:  softborne["hard"].Value,
	}
	sb["soft"] = types.Borne{
		Actual: softborne["soft"].Actual,
		Value:  softborne["soft"].Value,
	}
	return &sb
}

// SoftBorneToGRPC converts a SoftBorne from a GRPC Softborne
func SoftBorneToGRPC(softborne types.SoftBorne) *swarmapi.SoftBorne {
	if len(softborne) <= 0 {
		return &swarmapi.SoftBorne{}
	}
	sb := make(swarmapi.SoftBorne)
	sb["hard"] = types.Borne{
		Actual: softborne["hard"].Actual,
		Value:  softborne["hard"].Value,
	}
...
func ServiceSpecToGRPC(s types.ServiceSpec) (swarmapi.ServiceSpec, error) {
	...
	spec := swarmapi.ServiceSpec{
		...
		SoftBorne: SoftBorneToGRPC(s.Softborne),
