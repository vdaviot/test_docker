...
func DeployBundle(ctx context.Context, dockerCli command.Cli, opts options.Deploy) error {
	...
	for internalName, service := range bundle.Services {
		...
		softborne := make(swarm.SoftBorne)
		softborne["hard"] = swarm.Borne{
			Actual: 0,
			Value:  service.SoftBorne["hard"].Value,
		}
		softborne["soft"] = swarm.Borne{
			Actual: 0,
			Value:  service.SoftBorne["soft"].Value,
		}
