...
{{- end }}{{ end }}
{{- if .SoftBorne }}
  Hard: {{ with (index .SoftBorne "hard") }}{{ .Value }}{{ end }}
  Soft: {{ with (index .SoftBorne "soft") }}{{ .Value }}{{ end }}
{{- end }}
Service Mode:
...
func (ctx *serviceInspectContext) SoftBorne() swarm.SoftBorne {
	return ctx.Service.Spec.SoftBorne
}
