...
func Service(
	...
	softborne := convertSoftBorne(service.SoftBorne)
...
// Composetype a update
func convertSoftBorne(sb composetypes.SoftBorne) swarm.SoftBorne {
	if len(sb) <= 0 {
		return swarm.SoftBorne{}
	}
	newSb := make(swarm.SoftBorne)
	newSb["hard"] = swarm.Borne{
		Value:  sb["hard"].Value,
		Actual: 0,
	}
	newSb["soft"] = swarm.Borne{
		Value:  sb["soft"].Value,
		Actual: 0,
	}
	return newSb
